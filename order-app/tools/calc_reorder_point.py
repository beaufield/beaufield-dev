...
# Version: v1.0.0
#
# ⚠️ 2026-07-20 廃止（analyze_demand.py に統合）:
#   このスクリプトが書き込む「発注点マスター」（6ヶ月月平均のみの簡易ロジック）は、
#   analyze_demand.py（需要分析・Syntetos-Boylan分類・ABCランク差別化・減少トレンド判定）
#   が算出する「適正在庫」に一本化された。発注画面・在庫一覧・提案タブで基準を統一するため、
#   このスクリプトはもう実行しないこと。タスクスケジューラに登録している場合は無効化すること
#   （登録したまま残すと月次実行のたびに新しい適正在庫を古い簡易平均で上書きしてしまう）。
#
# 概要（旧仕様・参考用）:
...
def main():
    ...
    logging.error(
        '廃止済みスクリプトです。analyze_demand.py の需要分析ベース「適正在庫」に統一されたため、'
        'このスクリプトの実行は中止しました。タスクスケジューラに登録している場合は無効化してください。'
    )
    sys.exit(1)
