...
var BF_ = (function () {
// ===== BEGIN 本番 Code.gs（期限切れ行削除の1行だけ差し替え） =====
// ============================================================
  ...
  try {
    ...
    for (let i = 1; i < data.length; i++) {
      ...
      if (rowToken === token) {
        if (rowExpires < now) {
          // 期限切れ → 行を削除してから拒否
          /* 中継GASは読み取り専用: 期限切れ行の削除は本番GAS・ポータルに任せる */
...
// ===== END 本番 Code.gs =====
  return { adminBootstrap: adminBootstrap };
})();

// 中継ページ。許可した画面にだけ通信路（MessagePort）を渡す。
function doGet(e) {
  var p = (e && e.parameter) || {};
