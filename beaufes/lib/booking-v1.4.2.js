...
(function () {
  ...
  function mockShinbishinCatalog() {
    ...
    return products.flatMap(product=>times.map((time,index)=>({
      ...
      resource_group:product[0]==='H'?'':time[2],bullets:product[2],overview:product[3]
    })));
  }
  const mockCatalog = [
  ...
  async function request(url,action,data) {
      ...
      if(action==='reserveSessions'||action==='staffChangeReservation') {
        ...
        [...new Set(data.sessions)].forEach(id=>{
          ...
          if(occupied.some(item=>item.slot===s.slot || (s.resource_group&&item.resource_group===s.resource_group))){mockResult.invalid.push(id);return;}
           if(actor!=='staff'&&occupied.some(item=>sessionsOverlap(item,s))){mockResult.conflict.push(id);return;}
    ...
    let last;
    // 社員画面は一覧取得済みの内容を先に表示できるため、最新確認は再試行を2回に抑える。
    // 🔴 v1.4.1: 最新確認の打ち切りを12秒→45秒へ。GASの処理が完了していても、結果の配送
    //    （受け付け待ち・受け取り）が12秒の枠からはみ出して捨てられていた（2026-09-24実測）。
    //    8月の実測で12秒超の通信の半数以上は最終的に成功し、45秒までにその9割が届いていた。
    // 保存系は同じ操作IDで従来どおり長めに再試行し、結果不明のまま別操作にしない。
    const quickRead=action==='staffGetReservations';
    const attempts=quickRead?2:3;
    const timeoutMs=quickRead?45000:30000;
    // 同じdata・操作IDを再送する。タイムアウトしても別の予約操作にしない。
    for(let attempt=0;attempt<attempts;attempt++) {
      if(attempt)await new Promise(resolve=>setTimeout(resolve,quickRead?3000:attempt*1000));
  ...
  function mount(opts) {
    ...
    let current=null,ready=false,busy=false,confirming=false,resultUnknown=false,disposed=false,requestId=uid(),chosen=[],token=opts.token || '',pending=null,sequence=0;
    ...
    const loadBanner=node('div','','booking-load-banner');
    const loadSpinner=node('span','','booking-load-spinner');loadSpinner.setAttribute('aria-hidden','true');
    const loadMessage=node('span');
    loadBanner.hidden=true;loadBanner.setAttribute('role','status');loadBanner.setAttribute('aria-live','polite');loadBanner.append(loadSpinner,loadMessage);
    ...
    root.append(loadBanner,options,status);
    ...
    root.append(reload);
    function showLoadBanner(kind,message) {
      const visible=kind!=='none';
      loadBanner.hidden=!visible;
      loadBanner.className='booking-load-banner '+(visible?kind:'');
      loadSpinner.hidden=kind!=='loading';
    ...
    function render(data,isAuthoritative) {
      current=data;ready=isAuthoritative!==false;chosen=(data.reserved||[]).slice();options.replaceChildren();
      ...
      seminarSection.append(node('p','ビューフェス内で開催するセミナーです。セミナーのご参加にはご予約が必要です。','booking-help'));
      ...
      function renderCategory(container,categorySessions,isExperience) {
        ...
        if(group.length===1) {
          ...
          input.disabled=inputDisabled(s,reserved,busy||!ready);
        ...
        group.forEach(s=>{
          ...
          const input=node('input');input.type='checkbox';input.value=s.session_id;input.checked=reserved;
          input.disabled=inputDisabled(s,reserved,busy||!ready);input.dataset.bookingGroup=key;input.dataset.resourceGroup=s.resource_group||'';groupInputs.push(input);
      ...
      save.disabled=!ready || busy || !sessions.some(s=>s.can_book || (chosen.includes(s.session_id)&&s.can_cancel));
    }
    async function load() {
      if(disposed)return;
      const seq=++sequence;ready=false;resultUnknown=false;save.disabled=true;reload.disabled=true;status.textContent='空き状況と予約内容を確認しています…';
      showLoadBanner('loading','最新情報読み込み中');
      ...
      try{
        ...
        render(data,true);requestId=uid();pending=null;status.textContent='';showLoadBanner('none','');
        if(new URLSearchParams(location.search).has('semissue'))status.textContent='来場のお申し込みは完了しています。セミナー予約は下の現在の内容を確認し、必要ならお取りください。';
      }catch(e){
        if(disposed||seq!==sequence)return;
        if(opts.mode==='staff'&&['NO_TOKEN','NO_ROLE','SESSION_INVALID','USER_INACTIVE'].includes(e.message)&&typeof opts.onAuthError==='function')opts.onAuthError(e.message);
        const message=current
          ? '最新情報を取得できませんでした。表示内容は一覧取得時点です。保存はせず、「最新の予約を確認する」で再試行してください。'
          : (words[e.message]||'予約状況を確認できませんでした。来場申込・入場パスはそのまま利用できます。');
        status.textContent=message;
        showLoadBanner('error',message);
      }
      finally{if(!disposed&&seq===sequence)reload.disabled=false;}
    }
    ...
    save.addEventListener('click',async()=>{
      ...
      const removing=(current.reserved||[]).filter(id=>!chosen.includes(id));
       ...
       if(opts.mode==='staff') {
         const selected=(current.sessions||[]).filter(s=>chosen.includes(s.session_id));
         const hasOverlap=selected.some((s,i)=>selected.slice(i+1).some(other=>sessionsOverlap(s,other)));
         if(hasOverlap)confirmation+=' 時間が重なる予約を両方確保します。参加時間をお客様と調整してください。';
       }
      if(removing.length)confirmation+=' 選択を外したセミナー予約を取り消します。来場のお申し込みは取り消されません。';
      if(confirmation&&!await approve(confirmation))return;
      pending=pending||Object.assign(credentials(),payload());busy=true;resultUnknown=false;save.disabled=true;reload.disabled=true;options.querySelectorAll('input').forEach(x=>x.disabled=true);status.textContent='予約内容を保存しています…';
      try{
        const data=await request(opts.url,opts.mode==='staff'?'staffChangeReservation':'reserveSessions',pending);
        pending=null;requestId=uid();busy=false;resultUnknown=false;render(data,true);
        const r=data.result||{};
        ...
        status.textContent=words[e.message]||'保存結果を確認できませんでした。同じ内容で再試行するか、最新の予約を確認してください。';
        if(e.message==='BOOKING_CONFLICT'){ready=false;pending=null;}
        else if(!e.business||e.message==='BOOKING_RECOVERY_REQUIRED'||e.message==='INTERNAL_ERROR'){resultUnknown=true;ready=true;}
      }
      finally{
        busy=false;save.disabled=!ready;reload.disabled=false;
        if(ready&&!resultUnknown)options.querySelectorAll('input').forEach(x=>{const s=current.sessions.find(s=>s.session_id===x.value);x.disabled=x.checked?!s.can_cancel:(!s.can_book||s.remaining===0);});
        if(resultUnknown){options.querySelectorAll('input').forEach(x=>x.disabled=true);save.textContent='同じ内容でもう一度保存する';}
        else save.textContent='選択した内容で保存する';
      }
    });
    reload.addEventListener('click',load);
    // 社員一覧ですでに取得した情報があれば、通信を待たずにまず表示する。
    // 予約変更に必要な revision は専用APIで確認してから保存可能にする。
    if(opts.initialData){
      if(opts.mode==='staff'&&opts.initialDataAuthoritative===true){
        render(opts.initialData,true);status.textContent='';showLoadBanner('none','');
      }else{
        render(opts.initialData,false);
        status.textContent='一覧取得時点の予約内容を表示しています。最新情報を確認中です…';
        load();
      }
    }else load();
    return {payload:payload,setToken(t){token=t;return load();},load:load,dispose(){disposed=true;sequence++;}};
  }
  function staff(opts){
    const dialog=node('dialog','','booking-dialog'),close=node('button','閉じる','booking-reload'),root=node('div');close.type='button';
    let controller=null;
    dialog.append(close,root);document.body.append(dialog);close.addEventListener('click',()=>dialog.close());dialog.addEventListener('close',()=>{if(controller)controller.dispose();dialog.remove();});
    dialog.showModal();controller=mount(Object.assign({},opts,{root:root,mode:'staff'}));
  }
