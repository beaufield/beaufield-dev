...
(function () {
  ...
  async function request(url,action,data) {
    ...
    let last;
    // 社員画面は一覧取得済みの内容を先に表示できるため、最新確認だけを短い待ち時間で再試行する。
    // 保存系は同じ操作IDで従来どおり長めに再試行し、結果不明のまま別操作にしない。
    const quickRead=action==='staffGetReservations';
    const attempts=quickRead?2:3;
    const timeoutMs=quickRead?12000:30000;
    // 同じdata・操作IDを再送する。タイムアウトしても別の予約操作にしない。
    for(let attempt=0;attempt<attempts;attempt++) {
      if(attempt)await new Promise(resolve=>setTimeout(resolve,quickRead?500:attempt*1000));
